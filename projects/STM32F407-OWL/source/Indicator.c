...
static void sta_enter( indicator_sta_t sta ){
	...
	case INDICATOR_STA_STARTUP:
		break;
	case INDICATOR_STA_CONDITION_VIOLATED:
		led_setRGB( LED_STA, RGB_R );
		break;
	case INDICATOR_STA_IDLE_ALL_SYSTEM_GO:
		led_setRGB( LED_STA, RGB_G );
		break;
		
	case INDICATOR_STA_CONDITION_CHECKING:
		break;
	case INDICATOR_STA_SHUTTINGDOWN:
		led_setRGB( LED_STA, RGB_B );
		break;
	default:
...
static void sta_running( indicator_sta_t sta ){
	...
	case INDICATOR_STA_STARTUP:
		// scanningRGB( 200 );
		break;
	case INDICATOR_STA_FACTORY:
		blink_LED_STA( RGB_C, blinkflag_quick );
		break;
	case INDICATOR_STA_IDLE_ALL_SYSTEM_GO:
		break;
	case INDICATOR_STA_CONDITION_CHECKING:
		blink_LED_STA( RGB_Y, blinkflag_quick );
		break;
	default:
		break;
	}
...
void indicator_set( indicator_sta_t mode ){
	...
	switch( mode ){
	case INDICATOR_STA_FACTORY:
		isIntegratedLight = false;
		break;
	default:
		isIntegratedLight = true;
		break;
	}
...
static void prompt_enter( indicator_prompt_t sta ){
	switch( sta ){
	case INDICATOR_PROMPT_RC_COMMTEST:
		beep(30);
		break;
...
static void prompt_running( indicator_prompt_t sta ){
	switch( sta ){
	case INDICATOR_PROMPT_RC_COMMTEST:
		AllBlink( RGB_G, blinkflag_quick );
		break;
	case INDICATOR_PROMPT_RC_INVALID_COMMAND:
		AllBlink( RGB_Y, blinkflag_quick );
		break;
	default:
		break;
	}
...
static void integrated_enter( indicator_sta_t sta ){
	switch( sta ){
	case INDICATOR_STA_QUICKBLINK:
		led_allon();
		break;
	
	case INDICATOR_STA_SLOWBLINK:
		led_allon();
		break;
	
	case INDICATOR_STA_FADEOUT:
		led_allon();
		break;
	
	case INDICATOR_STA_FADEIN:
		led_allon();
		break;
	
	case INDICATOR_STA_BREATH:
		led_allon();
		break;
	...
	case INDICATOR_STA_BOUNCING:
		led_allon();
		break;

	case INDICATOR_STA_FILLING:
		led_allon();
		break;
...
static void integrated_running( indicator_sta_t sta ){
	...
	switch( sta ){
	case INDICATOR_STA_QUICKBLINK:
		AllBlink( blink_color, blinkflag_quick );
		break;
	case INDICATOR_STA_SLOWBLINK:
		AllBlink( blink_color, blinkflag_slow );
		break;
	case INDICATOR_STA_FADEOUT:
		AllFade();
		break;
